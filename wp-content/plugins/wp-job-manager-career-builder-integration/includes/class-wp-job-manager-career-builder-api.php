...
class WP_Job_Manager_Career_Builder_API {
	...
	private static function format_job( $raw_job ) {
		$job = array(
			...
			'tagline'         => sanitize_text_field( $raw_job['DescriptionTeaser']  ),
			'permalink'       => esc_url_raw( $raw_job['JobDetailsURL'] ),
			'location'        => sanitize_text_field( $raw_job['Location'] ),
			'lat'             => sanitize_text_field( $raw_job['LocationLatitude'] ),
			'lng'             => sanitize_text_field( $raw_job['LocationLongitude'] ),
			'type'            => sanitize_text_field( $raw_job['EmploymentType'] ),
			'type_slug'       => sanitize_title( $raw_job['EmploymentType'] ),
			'timestamp'       => strtotime( $raw_job['PostedDate'] ),
