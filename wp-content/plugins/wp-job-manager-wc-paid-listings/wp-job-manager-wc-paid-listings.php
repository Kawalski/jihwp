...
// Define constants
define( 'JOB_MANAGER_WCPL_VERSION', '2.2.1' );
...
function wp_job_manager_wcpl_init() {
	...
	class WP_Job_Manager_WCPL {
		...
		public function __construct() {
			...
			include_once( 'includes/class-wp-job-manager-wcpl-submit-job-form.php' );
			...
			if ( class_exists( 'WP_Resume_Manager' ) ) {
				...
				include_once( 'includes/class-wp-job-manager-wcpl-submit-resume-form.php' );
			}
		...
		public function settings( $settings = array() ) {
			$settings['job_submission'][1][] = array(
				...
				'std' 		=> '',
				'label' 	=> __( 'Paid Listings Flow', 'wp-job-manager-simple-paid-listings' ),
				'desc'		=> __( 'Define when the user should choose a package for submission.', 'wp-job-manager-wc-paid-listings' ),
				'type'      => 'select',
				'options'   => array(
					'' => __( 'Choose a package after entering job details', 'wc-job-manager-wc-paid-listings' ),
					'before' => __( 'Choose a package before entering job details', 'wc-job-manager-wc-paid-listings' )
				)
			);
			return $settings;
		}
